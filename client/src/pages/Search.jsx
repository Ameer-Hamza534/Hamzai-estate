...
const Search = () => {
  ...
  const handleChange = (e) => {
    ...
    if (e.target.id === "sort_order" && e.target.value === "") {
      setSidebarData({ ...sidebarData, sort: "createdAt", order: "desc" });
    }
  };
  ...
  const showMoreListing = async () => {
    ...
    const searchQuery = urlParams.toString();
    const res = await axios.get(`/api/listings/get${searchQuery}`);
    const data = res;
    if (data.length < 9) {
      setShowMore(false);
    }
  ...
  return (
    <div className="flex flex-col md:flex-row">
      <div className="sideBar p-7 md:border-r-2 border-b-2 md:min-h-screen">
      ...
      <div className="results flex-1">
        <h1 className="text-3xl font-semibold capitalize p-3">
          Listing results:
        </h1>
        <div className="p-7 flex flex-wrap gap-4">
          ...
          {showMore && (
            <div className="w-full flex justify-center">
              <button
                className="flex items-center gap-1 hover:underline hover:text-blue-700"
                onClick={showMoreListing}
              >
                Show more <FaArrowDown className="text-sm" />
              </button>
            </div>
          )}
        </div>
